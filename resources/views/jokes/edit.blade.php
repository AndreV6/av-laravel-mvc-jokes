                ...
                <div class="p-6 text-gray-900">
                    <div class="mb-6">
                        <a href="{{ route('jokes.show', $joke) }}"
                           class="text-blue-600 hover:underline">&larr; Back to Joke</a>
                    </div>

                    <form method="POST" action="{{ route('jokes.update', $joke) }}" class="space-y-6">
                        @csrf
                        @method('PATCH')
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-500">
                                Separate tags with commas (e.g., funny, pun, programming)
                            </p>
                            <x-input-error class="mt-2" :messages="$errors->get('tags')" />
                        </div>

                        <div class="flex items-center gap-4">
                            <x-primary-button>{{ __('Update Joke') }}</x-primary-button>
                            <a href="{{ route('jokes.show', $joke) }}"
